...
Http::FilterDataStatus Filter::decodeData(Buffer::Instance& data, bool end_stream) {
  ...
  processData(data, rq_handler_, config_->rqstats(), *decoder_callbacks_);

  return (end_stream || request_processing_finished_)
...
Http::FilterDataStatus Filter::encodeData(Buffer::Instance& data, bool end_stream) {
  ...
  processData(data, resp_handler_, config_->respstats(), *encoder_callbacks_);

  return (end_stream || response_processing_finished_)
             ? Http::FilterDataStatus::Continue
             : Http::FilterDataStatus::StopIterationAndBuffer;
}

void Filter::processData(Buffer::Instance& data, ThriftDecoderHandlerPtr& handler,
                         ThriftToMetadataStats& stats,
  ...
  TRY_NEEDS_AUDIT {
    ...
    handler->onData(data_copy, underflow);
    return;
  }
  ...
  stats.invalid_thrift_body_.inc();
}
